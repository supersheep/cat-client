...
if (!config.server) {
    ...
    createConnection(config.server.port, config.server.ip, false);
    var port = ":" + (config.server["http-port"] ? config.server['http-port'] : "8080");
    if (port == ":80") {
        port = "";
    }
    var url = "http://" + config.server.ip + port + "/cat/s/router?domain=" + config.domain + "&op=json&ip=" + config.ip;
