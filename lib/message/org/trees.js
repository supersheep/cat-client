...
var logger = require('../../logger.js')('trees'),
    sender = require('../io/sender'),
    Tree = require('./tree'),
    time = require('../util/time');

var debug = require("debug")("cat:tree");
...
function sendTree(uid, cb) {
    debug('Send message tree(curr uid = ' + uid + ').');
    ...
    try {
        ...
        logger.error('Send message tree(curr uid = ' + uid + ') failure. Unsupported error caught. ' + err);

        return cb || cb(err);
    }
    var serializeEndTime = new Date();
    debug('Serialize the message tree(curr uid = ' + uid + '), cost ' + time.durationInMicros(serializeBeginTime,
            serializeEndTime) + 'us, ' +
    ...
    sender.send(tree.serialize(), function(err) {
        ...
        debug('Send message tree(curr uid = ' + uid + ') success.');

        var removeBeginTime = new Date();
        removeTree(uid);
        var removeEndTime = new Date();
        debug('Clean up the message tree(curr uid = ' + uid + '), cost ' +
