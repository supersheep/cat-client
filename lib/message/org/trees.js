...
function sendTree(uid, cb) {
    logger.info('Send message tree(curr uid = %s).', uid);

    var tree = _getTree(uid);

    if (!tree) {
        logger.error('Send message tree(curr uid = %s) failure. ' +
            'Message tree not found.', uid);
        return cb || cb(err);
    }
    ...
    try {
        ...
        if (err instanceof exception.CatError) {
            logger.error(util.format('Send message tree(curr uid = %s) failure.', uid),
                err.message);
        } else if (err instanceof exception.CatFatal) {
            logger.fatal(util.format('Send message tree(curr uid = %s) failure.', uid),
                err.message);
        }
    ...
    logger.info('Serialize the message tree(curr uid = %s), cost %dus, ' +
        'message tree size %d bytes.', uid, time.durationInMicros(serializeBeginTime,
            serializeEndTime), treeSerialized.length);
    ...
    sender.send(tree.serialize(), function(err) {
        ...
        logger.info('Send message tree(curr uid = %s) success.', uid);

        var removeBeginTime = new Date();
        removeTree(uid);
        var removeEndTime = new Date();
        logger.info('Clean up the message tree(curr uid = %s), cost %dus.',
            uid, time.durationInMicros(removeBeginTime, removeEndTime));
    });
}
